...
class Configuracion(New_ventana):
    ...
    def recordar_actualizar_peso(self):
        try:
            ...
            # Cargar configuración de recordatorio
            cursor.execute("SELECT recordatorio, cantidad_dias FROM datos WHERE nombre = ?", (self.usuario,))
            config = cursor.fetchone()

            if config:
                estado, frecuencia = config
                if estado == "on":  # Solo procede si el recordatorio está activo
                    ...
                    # Verificar si corresponde mostrar el mensaje
            ...
            conn.commit()  # Asegurar que los cambios se guardan
            conn.close()
    ...
    def mostrar_mensaje_recordatorio_unavez(self, cursor):
        ...
        if self.ultimo_msj != fecha_hoy:
            ...
            self.ultimo_msj = fecha_hoy  # Actualizamos la fecha para evitar duplicados

            # Desactivar el recordatorio en la base de datos
            cursor.execute("""
                UPDATE datos 
